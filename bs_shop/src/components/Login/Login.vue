        ...
        <form>
          ...
          <div :class="{current: loginMode}">
            ...
            <section class="login-hint">
              温馨提示：未注册商城帐号的手机号，登录时将自动注册，且代表已同意
          ...
          <div :class="{current: !loginMode}">
            <section>
              <section class="login-message">
                <input type="text" maxlength="11" placeholder="用户名/邮箱" v-model="user_name">
              </section>
                <section class="login-message">
                <input type="text" maxlength="11" placeholder="电话" v-model="phone">
              </section>
              <section class="login-verification">
                <input type="password" maxlength="18" placeholder="密码" v-if="pwdMode" v-model="pwd" autocomplete>
                <input type="text" maxlength="18" placeholder="密码" v-else v-model="pwd" autocomplete>
                <!-- 密码明文密文显示 -->
...
<script>
  ...
  import bgImg from './images/timgF0FCAPB52.jpg'
  ...
  export default {
    ...
    data() {
      return {
        ...
        captcha: '',  // 图形验证码
        bgImg: {
                backgroundImage: "url(" + require("./images/timgF0FCAPB52.jpg") + ")",
    ...
    methods: {
      ...
      async login() {
        ...
        if (this.loginMode) { // 验证码登录
          ...
          // 5.5 用户名和密码的登录
          const result = await pwdLogin(this.user_name, this.pwd, this.phone,this.captcha);
          // console.log(result);
          if (result.success_code === 200) {
            this.userInfo = result.message;
              console.log("登入成功")
...
<style scoped lang="stylus" rel="stylesheet/stylus">
  ...
  .login-container
    width 100%
    height 100%
    background-image url ('./images/timgF0FCAPB52.jpg')
    .login-inner
      padding-top 60px
      width 80%
      margin 0 auto
